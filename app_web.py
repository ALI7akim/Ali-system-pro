...
# إدارة الانتقال بين الصفحات والـ Keys
...
# ==============================================================================
    ...
    def on_barcode_change():
        ...
        # تنظيف المدخلات للتأكد من أنها أرقام فقط
    ...
    # عرض خانة الباركود وخانة الكمية على نفس الخط (سطر واحد متناسق)
    col_inputs1, col_inputs2 = st.columns([2, 1])
    
    with col_inputs1:
        # تحويلها لـ text_input لضمان استقبال المدخلات الرقمية السريعة بثبات
        barcode_val = st.text_input("🔍 امسح الباركود أو اكتب الكود (أرقام فقط) واضغط Enter:", 
                        value="",
                        key=f"barcode_input_{st.session_state.barcode_key}", 
                        on_change=on_barcode_change)
        
    with col_inputs2:
        # حقل الكمية يدعم الكسور العشرية بشكل مرن
    ...
    # حقن كود جافاسكربت عبر بيئة معزولة ومضمونة ترتبط مباشرة بـ واجهة المستخدم بالموقع الإلكتروني
    components.html(f"""
        <script>
        ...
        // تكرار الفحص للتأكد من استقرار تحميل الخانات بالصفحة
    ...
    if st.session_state.active_item:
        ...
        st.write("")
        # الحفظ التلقائي عند نقر إنتر في الكمية
        if st.button("💾 حفظ الصنف إلى القائمة والانتقال للمسح التالي (Enter)", type="primary", use_container_width=True, key=f"save_btn_{st.session_state.barcode_key}") or qty_input > 0:
            ...
            # الانتقال للمسح التالي وتصفير الحقول وإعادة توجيه التركيز للباركود فوراً
            st.session_state.active_item = None
    ...
    if current_list:
        ...
        if show_preview:
            ...
            for idx, entry in enumerate(current_list):
                ...
                with col_i1:
                    st.markdown(f"<div style='padding-top:5px; font-size:13px;'><b>{entry['Name']}</b><br><small style='color:#666;'>SAP: {entry['SAP']} | الشركة: {entry.get('Supplier', '-')}</small></div>", unsafe_allow_html=True)
                with col_i2:
