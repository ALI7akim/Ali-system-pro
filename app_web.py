...
with st.sidebar:
    ...
    if st.session_state.master_df is not None: st.success("📁 ملف الباركودات: مَحْفُوظ وجاهز")
    else: st.error("❌ ملف الباركودات: غير متوفر")
        
    if st.session_state.stock_df is not None: st.success("📁 ملف المخزون: مَحْفُوظ وجاهز")
    else: st.error("❌ ملف المخزون: غير متوفر")
    
    st.divider()
    st.markdown("### 🔄 رفع ملفات جديدة لتحديث النظام:")
...
# حماية التشغيل إذا كان البرنامج يفتح لأول مرة تماماً وبدون أي ملفات مخزنة
if st.session_state.master_df is None or st.session_state.stock_df is None:
    st.warning("⚠️ يرجى رفع ملف الباركودات وملف المخزون من القائمة الجانبية (لمرة واحدة فقط) لحفظها وتفعيل النظام.")
    st.stop()
...
# ==============================================================================
    ...
    # سطر علوي للعودة واستعراض معلومات الوجهة الحالية
    ...
    if st.session_state.active_item:
        ...
        st.markdown(f"""
            ...
            <p class="computer-label">Description (اسم الصنف):</p><div style="background-color: white; border: 1px solid #aaa; padding: 6px; font-size: 14px; margin-bottom: 8px; font-weight: bold; color: #111;">{item['Name']}</div>
        ...
        st.markdown("<p style='text-align:center; color:#777; padding:15px;'>النظام جاهز ومستعد لمسح الصنف التالي...</p>", unsafe_allow_html=True)
